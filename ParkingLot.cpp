...
bool ParkingLot::findPlate(const std::string& plate, ParkingSlot*& slot) {
    for (int b = 0; b < floors; ++b)
        for (int r = 0; r < rows; ++r)
            for (int c = 0; c < cols; ++c)
...
void ParkingLot::parkCar(ParkingSlot* slot, const std::string& plate) {
...
const std::vector<std::vector<std::vector<ParkingSlot>>>& ParkingLot::getAllSlots() const { return slots; }
std::vector<std::vector<std::vector<ParkingSlot>>>& ParkingLot::getAllSlots() {
    return slots;
}
