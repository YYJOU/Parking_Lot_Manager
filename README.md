# 系級班級 : 資工 1A
# 成員資訊 : 周貽洋、陳柏鈞、林湧晉
# 小專題題目 : 停車場管理系統

# 專案簡介：
### 管理多層樓停車場的車位、車輛進出、月租車、收費與資料保存

# 製作動機：
### 靈感來自於大孝館的停車空間配置，發現管理上有許多可以優化的地方，因此決定以「停車場管理系統」作為出發點，設計一套完整的車輛進出場登記、收費計算及車位狀態即時更新的模擬系統，並結合實際需求進行功能延伸與應用。

# 主要功能：
### ● 車輛進出場
### ● 月租車管理
### ● 停車場狀態查詢
### ● 管理員模式（含密碼）
### ● 收費與盈收統計
### ● 資料匯入/匯出

# 如何執行程式：
### 1. 在github點開Code
### 2. 點擊Download進行下載
### 3. 於Visual Sudio開啟程式
### 4. 在畫面中找到檔案
### 5. 開啟後即完成安裝
### 6. 打開後點選 本機WINDOWS偵錯工具
![image](https://github.com/user-attachments/assets/1c9ced28-493f-46ea-9757-f98a9f3102ab)
### 7. 即可開始執行

# 程式畫面截圖：
![擷取](https://github.com/user-attachments/assets/9f3d64f1-6484-4b3b-aca5-5d7478d3016e)
![擷取1](https://github.com/user-attachments/assets/b0518feb-1327-4941-937b-045267cba48f)
![擷取2](https://github.com/user-attachments/assets/e26219e6-9856-4d0f-8231-cd094203d612)
![擷取3](https://github.com/user-attachments/assets/31518583-1a3c-40df-b2be-0261394d8c0f)
![擷取4](https://github.com/user-attachments/assets/0d1fd519-3da8-4381-9c7b-fc244bb4e3b0)

# 分工資訊：
### 周貽洋(組長)：程式設計、封裝、撰寫、簡報製作、影片錄製、README製作
### 陳柏鈞：程式設計、簡報製作、影片錄製、README製作
### 林湧晉：程式測試、提供想法
